...
struct GraphicsGeometry {
  size_t vertex_positions_buffer_size() const {
    return vertex_positions.size() * sizeof(Point3f);
  }
  size_t vertex_normals_buffer_size() const {
    return vertex_normals.size() * sizeof(Vector3f);
  }
  size_t vertex_colors_buffer_size() const {
    return vertex_colors.size() * sizeof(Point4f);
  }
  size_t indices_buffer_size() const {
