...
public:
  ...
  void set_highlight_color(const Point4f& color) { render_object2_.set_color(color); }

  void render(const std::unique_ptr<ShaderProgram>& shader_program) const;
  void render_highlight(const std::unique_ptr<ShaderProgram>& shader_program) const;
